...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
############ PROPOSED ALTERNATIVE MODEL OUTPUT WITH INTERACTION OF AGE AND MIGHRATION #############################
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

### CALCULATE PREDICTED SURVIVAL BASED ON MODEL with binary additive parameters
## MIG STAGES ARE: 0=stationary, 1=migratory
## POPULATION CLASSES ARE: pop1: 1=western Europe, 0=elsewhere
## pop2: 1= Italy/Balkans, 0= elsewhere
...
AnnTab<-data.frame(pop1=rep(c(0,1,0), each=24),
                   pop2=rep(c(1,0,0), each=24),
                   capt=0,
...
Xin<-AnnTab %>% mutate(capt=1) %>% bind_rows(AnnTab) %>% 
  mutate(Population=ifelse(pop1==1,"western Europe",ifelse(pop2==1,"Balkans/Italy","Caucasus/Middle East")))
...
for(s in 1:nrow(MCMCout)) {
  ...
  X<-  Xin %>%
    ...
    mutate(logit.surv=as.numeric(MCMCout[s,match("lp.mean",parmcols)])+
             ifelse(age==0,as.numeric(MCMCout[s,match("b.phi.mig[1]",parmcols)]),as.numeric(MCMCout[s,match("b.phi.mig[2]",parmcols)]))*mig +
             as.numeric(MCMCout[s,match("b.phi.age",parmcols)])*age +
             as.numeric(MCMCout[s,match("b.phi.capt",parmcols)])*capt +
             as.numeric(MCMCout[s,match("b.phi.pop1",parmcols)])*pop1 +
...
MigTab<-expand.grid(age=c(0,1),mig=c(0,1),pop1=c(1,0), pop2=c(1,0), capt=c(0,1)) %>%
  mutate(Population=ifelse(pop1==1,"western Europe",ifelse(pop2==1,"Italy/Balkans","Caucasus/Middle East")))
