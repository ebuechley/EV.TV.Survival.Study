...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ADJUST PHI MATRIX BASED ON AREA >30 N to distinguish summer and winter residence and long >9 to distinguish migration in E and W
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
dim(lat.matrix)
...
for(row in 1:nrow(EV.phi.matrix)) {
  for(col in 2:ncol(EV.phi.matrix)) {
    if(EV.phi.matrix[row,col]==1){
      EV.phi.matrix[row,col]<-ifelse(lat.matrix[row,col]>30,1,3)
      EV.phi.matrix[row,col]<-ifelse(is.na(lat.matrix[row,col]),1,EV.phi.matrix[row,col])
    }
    if(EV.phi.matrix[row,col]==2){
      EV.phi.matrix[row,col]<-ifelse(long.orig[row,2]>9 & long.orig[row,2]<35,2,4)
    }
  }
}
...
parameters.telemetry <- c("p.seen.alive","base.obs","base.fail","base.recover","beta1","beta2","beta3","beta4",
                          "mean.phi","lp.mean","b.phi.age","b.phi.mig","b.phi.capt","b.phi.resident")
...
# # Call JAGS from R (took 70 min DIC = 3928.403)
EGVU_surv_mod <- autojags(INPUT.telemetry, inits.telemetry, parameters.telemetry,
                     "C:\\STEFFEN\\RSPB\\Bulgaria\\Analysis\\Survival\\EV.TV.Survival.Study\\EGVU_telemetry_survival_nolatlong.jags",
                     n.chains = nc, n.thin = nt, n.burnin = nb, n.cores=nc, parallel=T) #, n.iter = ni)



# Call JAGS from R (took 70 min DIC = 3912.238)
EGVU_surv_resident <- autojags(INPUT.telemetry, inits.telemetry, parameters.telemetry,
                          "C:\\STEFFEN\\RSPB\\Bulgaria\\Analysis\\Survival\\EV.TV.Survival.Study\\EGVU_telemetry_survival_nolatlong_resident.jags",
                          n.chains = nc, n.thin = nt, n.burnin = nb, n.cores=nc, parallel=T) #, n.iter = ni)



##### CHANGE INPUT DATA FOR RESIDENT POPULATION AND RERUN MODEL #####
for(row in 1:nrow(EV.phi.matrix)) {
    if(INPUT.telemetry$resid[row]==0){
      INPUT.telemetry$mig[row,]<-5
    }
}



# Call JAGS from R (took 70 min DIC = 3925.252)
EGVU_surv_res_mig <- autojags(INPUT.telemetry, inits.telemetry, parameters.telemetry,
                          "C:\\STEFFEN\\RSPB\\Bulgaria\\Analysis\\Survival\\EV.TV.Survival.Study\\EGVU_telemetry_survival_nolatlong.jags",
                          n.chains = nc, n.thin = nt, n.burnin = nb, n.cores=nc, parallel=T) #, n.iter = ni)
...
save.image("EGVU_survival_output_extended_935EW.RData")
...
sink("EGVU_telemetry_survival_nolatlong_resident.jags")
cat("
    model {
