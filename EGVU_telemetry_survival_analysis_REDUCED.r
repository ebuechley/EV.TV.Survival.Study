...
sink("EGVUsurv_simplage_5migstage.jags")
cat("
    model {
  ...
  for (i in 1:nind){
    for (t in f[i]:(n.occasions)){
        logit(phi[i,t]) <- lp.mean[adult[i,t]+1] +      ### age category-specific intercept 
...
# Specify model in BUGS language
sink("EGVUsurv_simplage_2migstage_intpop.jags")
cat("
  model {
    ...
    for (i in 1:nind){
      for (t in f[i]:(n.occasions)){
        logit(phi[i,t]) <- lp.mean[adult[i,t]+1] +      ### age category-specific intercept 
          b.phi.mig*mig[i,t] +       ### survival dependent on migratory stage of the month (stationary or migratory)
          b.phi.pop[pop[i]] +       ### survival dependent on migratory stage of the month (stationary or migratory)
          b.phi.capt*(capt[i])      ### survival dependent on captive-release (wild or captive-raised)
      } #t
    } #i
    ...
    #### CATEGORICAL POPULATION OFFSET
    for(pop in 1:3){
      b.phi.pop[pop] ~ dnorm(0, 0.001)               # Prior for population on survival probability on logit scale
    }

    #### SLOPE PARAMETERS FOR SURVIVAL PROBABILITY
    b.phi.mig ~ dnorm(0, 0.001)           # Prior for COST OF MIGRATION migration on survival probability on logit scale
...
# Specify model in BUGS language
sink("EGVUsurv_simplage_4migstage.jags")
cat("
  model {
...
cat("
  model {
    ...
    b.phi.vul ~ dnorm(0, 0.001)         # Prior for vulnerable state on survival probability on logit scale
